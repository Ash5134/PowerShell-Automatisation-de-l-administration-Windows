...
while ($true) {
    ...
    Write-Host " 1 - Config OU" -ForegroundColor Yellow

    # Affiche l'option 2 : configurer les utilisateurs.
    Write-Host " 2 - Config USER" -ForegroundColor Yellow

    # Affiche l'option 3 : configurer les groupes.
    Write-Host " 3 - Config Groupe" -ForegroundColor Yellow

    # Affiche l'option 4 : configurer les stratégies de groupe (GPO).
    Write-Host " 4 - Config GPO" -ForegroundColor Yellow

    # Affiche l'option 5 : gérer les journaux d'événements (logs Windows).
    Write-Host " 5 - Gérer Événements" -ForegroundColor Yellow

    # Affiche l'option 6 : gérer les processus en cours.
    Write-Host " 6 - Gérer Processus" -ForegroundColor Yellow

    # Affiche l'option 7 : gérer les services (Windows Services).
    Write-Host " 7 - Gérer Services" -ForegroundColor Yellow

    # Affiche l'option 8 : supervision corrective (par exemple, redémarrer un service arrêté).
    Write-Host " 8 - Supervision corrective" -ForegroundColor Yellow
    ...
    if ($maVar -eq "0") {
        ...
        Write-Host " Fermeture du programme. À bientôt."
    ...
    while (-not $exitOUMenu) {
        ...
        switch ($choix_ou) {
            ...
            "1" {
                ...
                Get-ADOrganizationalUnit -Filter * -SearchBase "DC=beweb2,DC=local" |
                ...
                $ouExist = Get-ADOrganizationalUnit -Filter "Name -eq '$nom_ou'" -SearchBase "DC=beweb2,DC=local" -ErrorAction SilentlyContinue

                # Si aucune OU du même nom n'existe, on procède à la création
                if (-not $ouExist) {
                    try {
                        # Crée l'OU dans l'annuaire Active Directory
                        New-ADOrganizationalUnit -Name $nom_ou -Path "DC=beweb2,DC=local" -ErrorAction Stop
            ...
            "2" {
                ...
                $ouToDelete = Get-ADOrganizationalUnit -Filter "Name -eq '$nom_ou'" -SearchBase "DC=beweb2,DC=local" -ErrorAction SilentlyContinue
...
elseif ($maVar -eq "2") {
    ...
    while ($true) {
        ...
        Write-Host " 3 - Affectation OU" -ForegroundColor Yellow
        Write-Host " 4 - Affectation groupe" -ForegroundColor Yellow
        Write-Host " 5 - Gestion ACL`n" -ForegroundColor Yellow
        ...
        elseif ($choix_user -eq "1") {
            ...
            Get-ADOrganizationalUnit -SearchBase "DC=beweb2,DC=local" -Filter * |
                Select-Object -ExpandProperty Name |
                ForEach-Object { Write-Host " - $_" }
            ...
            New-ADUser -Name $nom_user -SamAccountName $login_user -UserPrincipalName "${login_user}@beweb2.local" `
                -AccountPassword $mdp -Enabled $true -Path "OU=$ou,DC=beweb2,DC=local"
        ...
        elseif ($choix_user -eq "3") {
            ...
            try {
                ...
                Move-ADObject -Identity $user.DistinguishedName -TargetPath "OU=$ou,DC=beweb2,DC=local"
        ...
        # === OPTION 5 : GESTION DES DROITS D'ACCÈS (ACL) SUR DOSSIER ===
        elseif ($choix_user -eq "5") {
            ...
            try {
                ...
                $rule = New-Object System.Security.AccessControl.FileSystemAccessRule(
                    "beweb2\$user",                          # Utilisateur concerné
...
elseif ($maVar -eq "3") {
    ...
    function Get-GroupTypeDescription {
        ...
        $scope = switch ($groupType -band 0x00000003) {
            0x00000000 { "Local" }         # Portée locale au domaine
    ...
    while ($choix_grp -ne "0") {
        ...
        switch ($choix_grp) {
            ...
            "1" {
                ...
                # Dictionnaire pour traduire les types saisis en français vers la syntaxe AD
                $scopeOptions = @{ "local" = "DomainLocal"; "global" = "Global"; "universel" = "Universal" }
                ...
                while (-not $scopeOptions.ContainsKey($type_grp.ToLower())) {
                    $type_grp = Read-Host "Type de groupe (Local / Global / Universel)"
                }
                # Récupère la version AD du scope (ex : DomainLocal)
                ...
                try {
                    ...
                    New-ADGroup -Name $nom_grp -GroupScope $type_grp_ad -GroupCategory $cat_grp_ad -Path "CN=Users,DC=beweb2,DC=local"
...
elseif ($maVar -eq "4") {
    ...
    Write-Host "===== CONFIGURATION GPO - PARAMÈTRES DÉDIÉS =====" -ForegroundColor Cyan
    ...
    $ouToLink = Read-Host "Merci de saisir le DistinguishedName (DN) de l'OU cible (exemple : OU=Utilisateurs,DC=beweb2,DC=local)"
    ...
    function CreateAndConfigureGPO($gpoName) {
        ...
        switch ($gpoName) {
            ...
            "GPO_DeployWallpaper" {
                Set-GPRegistryValue -Name $gpoName `
                    -Key "HKCU\Control Panel\Desktop" `
                    -ValueName "Wallpaper" -Type String -Value "C:\Wallpapers\tssr.jpg"
                Set-GPRegistryValue -Name $gpoName `
                    -Key "HKCU\Control Panel\Desktop" `
    ...
    foreach ($idx in $selectedIndexes) {
        ...
        # On tente de lier cette GPO à l'OU cible saisie plus tôt
        try {
...
elseif ($maVar -eq "5") {
    ...
    function Watch-FailedLogons {
        ...
        function OnEventDetected {
            ...
            # Si pas d'IP (par exemple en local), on affiche N/A
...
elseif ($maVar -eq "6") {
    ...
    # Définit le chemin du dossier où seront stockés les logs
        ...
        do {
            ...
            switch ($choice) {
                ...
                "5" {
                    ...
                    # Variable pour contrôler la boucle de surveillance
...
elseif ($maVar -eq "7") {
    ...
    function Show-ServiceManager {
        ...
        # Liste des services critiques à surveiller sur un contrôleur de domaine
        $servicesCritiques = @(
        ...
        # Définit le chemin du dossier de rapport, organisé par année et mois (ex: C:\Rapports\2025\07)
...
"@ | Out-File $rapportTxt  # Enregistre ce bloc dans le fichier texte
        ...
        switch ($choice) {
            ...
            "3" {
                ...
                try {
                    # Essaie d'arrêter le service, en forçant l'arrêt si nécessaire
...
if ($maVar -eq "8") {
    ...
    Function Write-Log {
        ...
        # Convertit la chaîne en type d'entrée d'événement Windows
